...
import io.github.ghosthack.seven.internal.xz.DeltaInputStream;
...
final class DeltaDecoder extends AbstractCoder {
    ...
    InputStream decode(final String archiveName, final InputStream in, final long uncompressedLength, final Coder coder, final byte[] password,
        ...
        return new DeltaInputStream(in, getDistance(coder));
    }

    private int getDistance(final Coder coder) {
        if (coder == null || coder.properties == null || coder.properties.length == 0) {
            return 1;
        }
