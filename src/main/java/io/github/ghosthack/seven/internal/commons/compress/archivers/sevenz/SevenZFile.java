 ...
 * io.github.ghosthack.seven.internal.commons.compress.utils.MultiReadOnlySeekableByteChannel} for example.
 * </p>
 *
 * @NotThreadSafe
 * @since 1.6
...
public class SevenZFile implements Closeable {
    ...
    /** Opens the one strict, indexed reader configuration used by robust-seven. */
    public static SevenZFile open(
        ...
        final byte[] encodedPassword =
                password == null
                        ? null
                        : AES256SHA256Decoder.utf16Decode(password.clone());
        try {
            return new SevenZFile(
                    channel, archiveName, encodedPassword, maxMemoryLimitKiB);
        } finally {
    ...
    private static void drain(final InputStream input) throws IOException {
        final byte[] buffer = new byte[8192];
        while (input.read(buffer) >= 0) {
            // Drain the preceding solid-folder entry.
        }
    }

    private static byte[] readRange(final InputStream input, final int length)
            throws IOException {
        ...
        return offset == length ? result : Arrays.copyOf(result, offset);
    }
    ...
    private SevenZFile(final SeekableByteChannel channel, final String fileName,
            final byte[] password, final int maxMemoryLimitKiB) throws IOException {
    ...
    /** Builds the indexed decoding stream for one entry. */
    private void buildDecodingStream(final int entryIndex) throws IOException {
        ...
        if (currentFolderIndex == folderIndex) {
            ...
            reopenFolderInputStream(folderIndex);
        }

        final boolean haveSkippedEntries =
                skipEntriesWhenNeeded(entryIndex, isInSameFolder, folderIndex);

        if (currentEntryIndex == entryIndex && !haveSkippedEntries) {
            return;
        }

        InputStream fileStream =
                new BoundedInputStream(
                        currentFolderInputStream, file.getSize(), false);
        if (file.getHasCrc()) {
            fileStream =
                    new ChecksumInputStream(
                            fileStream, new CRC32(), -1, file.getCrcValue());
        }
    ...
    private void calculateStreamMap(final Archive archive) throws IOException {
        ...
        final int numFolders = archive.folders == null ? 0 : archive.folders.length;
        ...
        final List<List<SevenZMethod>> folderMethods =
                new ArrayList<>(numFolders);
        for (int folderIndex = 0; folderIndex < numFolders; folderIndex++) {
            final LinkedList<SevenZMethod> methods = new LinkedList<>();
            for (final Coder coder : archive.folders[folderIndex].getOrderedCoders()) {
                if (coder.numInStreams != 1 || coder.numOutStreams != 1) {
                    throw new IOException("Multi input/output stream coders are not yet supported");
                }
                methods.addFirst(SevenZMethod.byId(coder.decompressionMethodId));
            }
            folderMethods.add(List.copyOf(methods));
        }
        for (int entryIndex = 0; entryIndex < archive.files.length; entryIndex++) {
            final int folderIndex = fileFolderIndex[entryIndex];
            archive.files[entryIndex].setContentMethods(
                    folderIndex < 0 ? List.of() : folderMethods.get(folderIndex));
        }
    }
    ...
    private InputStream getCurrentStream() throws IOException {
        ...
        if (deferredBlockStreams.isEmpty()) {
            throw new IllegalStateException("No current 7z entry stream");
        }
     ...
     * @since 1.20
     */
    public InputStream getInputStream(final int entryIndex) throws IOException {
        compressedBytesReadFromCurrentEntry = 0;
        if (entryIndex < 0 || entryIndex >= archive.files.length) {
            throw new IndexOutOfBoundsException(entryIndex);
        }
    ...
    /** Returns the immutable-order archive index. */
    public List<SevenZArchiveEntry> entries() {
        return List.copyOf(Arrays.asList(archive.files));
    }
    ...
    /** Returns compressed bytes consumed by the current indexed stream. */
    ...
    private ByteBuffer readEncodedHeader(final ByteBuffer header, final Archive archive, final byte[] password) throws IOException {
        ...
        final byte[] nextHeader = readRange(inputStreamStack, unpackSize);
    ...
    private void readFilesInfo(final ByteBuffer header, final Archive archive) throws IOException {
        ...
        while (true) {
            ...
            case NID.kEmptyFile: {
                isEmptyFile = readBits(header, requireNonNull(isEmptyStream, "isEmptyStream for " + archive).cardinality());
                break;
            }
            case NID.kAnti: {
                isAnti = readBits(header, requireNonNull(isEmptyStream, "isEmptyStream for " + archive).cardinality());
                break;
            }
            case NID.kName: {
    ...
    private Archive readHeaders(final byte[] password) throws IOException {
        ...
        if (!headerLooksValid) {
            throw new IOException("archive has no valid start header");
        }
    ...
    private StartHeader readStartHeader(final long startHeaderCrc) throws IOException {
        ...
        try (DataInputStream dataInputStream =
                new DataInputStream(
                        new ChecksumInputStream(
                                new BoundedSeekableByteChannelInputStream(channel, 20),
                                new CRC32(),
                                20L,
                                startHeaderCrc))) {
     ...
     */
    private void reopenFolderInputStream(final int folderIndex) throws IOException {
        deferredBlockStreams.clear();
    ...
    private boolean skipEntriesWhenNeeded(final int entryIndex, final boolean isInSameFolder, final int folderIndex) throws IOException {
        ...
        for (int i = filesToSkipStartIndex; i < entryIndex; i++) {
            ...
            InputStream fileStreamToSkip =
                    new BoundedInputStream(
                            currentFolderInputStream, fileToSkip.getSize(), false);
