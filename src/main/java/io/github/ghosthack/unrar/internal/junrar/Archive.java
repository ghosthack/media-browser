...
import io.github.ghosthack.unrar.internal.junrar.crc.RarCRC;
import io.github.ghosthack.unrar.internal.junrar.crypt.Rar5Crypt;
import io.github.ghosthack.unrar.internal.junrar.crypt.Rijndael;
import io.github.ghosthack.unrar.internal.junrar.exception.BadRarArchiveException;
import io.github.ghosthack.unrar.internal.junrar.exception.CorruptHeaderException;
import io.github.ghosthack.unrar.internal.junrar.exception.CrcErrorException;
import io.github.ghosthack.unrar.internal.junrar.exception.HeaderNotInArchiveException;
import io.github.ghosthack.unrar.internal.junrar.exception.InitDeciphererFailedException;
import io.github.ghosthack.unrar.internal.junrar.exception.MainHeaderNullException;
import io.github.ghosthack.unrar.internal.junrar.exception.MissingPreviousVolumeException;
import io.github.ghosthack.unrar.internal.junrar.exception.NotRarArchiveException;
import io.github.ghosthack.unrar.internal.junrar.exception.RarException;
import io.github.ghosthack.unrar.internal.junrar.exception.UnsupportedRarEncryptedException;
import io.github.ghosthack.unrar.internal.junrar.exception.UnsupportedRarMethodException;
import io.github.ghosthack.unrar.internal.junrar.exception.UnsupportedRarVersionException;
import io.github.ghosthack.unrar.internal.junrar.exception.WrongPasswordException;
import io.github.ghosthack.unrar.internal.junrar.io.Raw;
import io.github.ghosthack.unrar.internal.junrar.io.RawDataIo;
import io.github.ghosthack.unrar.internal.junrar.io.SeekableReadOnlyByteChannel;
import io.github.ghosthack.unrar.internal.junrar.rarfile.AVHeader;
import io.github.ghosthack.unrar.internal.junrar.rarfile.BaseBlock;
import io.github.ghosthack.unrar.internal.junrar.rarfile.BlockHeader;
import io.github.ghosthack.unrar.internal.junrar.rarfile.CommentHeader;
import io.github.ghosthack.unrar.internal.junrar.rarfile.EAHeader;
import io.github.ghosthack.unrar.internal.junrar.rarfile.EndArcHeader;
import io.github.ghosthack.unrar.internal.junrar.rarfile.FileHeader;
import io.github.ghosthack.unrar.internal.junrar.rarfile.MacInfoHeader;
import io.github.ghosthack.unrar.internal.junrar.rarfile.MainHeader;
import io.github.ghosthack.unrar.internal.junrar.rarfile.MarkHeader;
import io.github.ghosthack.unrar.internal.junrar.rarfile.ProtectHeader;
import io.github.ghosthack.unrar.internal.junrar.rarfile.Rar5FileHeaderReader;
import io.github.ghosthack.unrar.internal.junrar.rarfile.SignHeader;
import io.github.ghosthack.unrar.internal.junrar.rarfile.SubBlockHeader;
import io.github.ghosthack.unrar.internal.junrar.rarfile.SubBlockHeaderType;
import io.github.ghosthack.unrar.internal.junrar.rarfile.UnixOwnersHeader;
import io.github.ghosthack.unrar.internal.junrar.rarfile.UnrarHeadertype;
import io.github.ghosthack.unrar.internal.junrar.rarfile.rar5.Rar5BaseBlock;
import io.github.ghosthack.unrar.internal.junrar.rarfile.rar5.Rar5BlockType;
import io.github.ghosthack.unrar.internal.junrar.rarfile.rar5.Rar5HashType;
import io.github.ghosthack.unrar.internal.junrar.rarfile.rar5.Rar5MainHeader;
import io.github.ghosthack.unrar.internal.junrar.unpack.ComprDataIO;
import io.github.ghosthack.unrar.internal.junrar.unpack.Unpack;
import io.github.ghosthack.unrar.internal.junrar.unpack.Unpack5;
import io.github.ghosthack.unrar.internal.junrar.volume.FileVolumeManager;
import io.github.ghosthack.unrar.internal.junrar.volume.InputStreamVolumeManager;
import io.github.ghosthack.unrar.internal.junrar.volume.Volume;
import io.github.ghosthack.unrar.internal.junrar.volume.VolumeManager;
...
public class Archive implements Closeable, Iterable<FileHeader> {
    ...
    public Archive(final VolumeManager volumeManager, final ArchiveOptions options)
        ...
        this.maxHeaders = options.getMaxHeaders();
    ...
    /**
     * Opens an extraction session using this archive's already parsed initial-volume index.
     *
     * <p>The returned archive owns an independent channel and unpack state. Header objects are
     * shared read-only until a multi-volume extraction advances the returned session, at which
     * point that session replaces only its own header list.
     */
    public Archive fork(final File firstVolume, final ArchiveOptions options) throws IOException {
        return new Archive(new FileVolumeManager(firstVolume), options, this, true);
    }

    private Archive(
            final VolumeManager volumeManager,
            final ArchiveOptions options,
            final Archive parsedIndex,
            final boolean forkedSession)
            throws IOException {
        this.volumeManager = volumeManager;
        this.unrarCallback = options.getUnrarCallback();
        this.passwordChars = options.getPassword();
        ...
        this.headers.addAll(parsedIndex.headers);
        this.markHead = parsedIndex.markHead;
        this.format = parsedIndex.format;
        this.rar5HeadersEncrypted = parsedIndex.rar5HeadersEncrypted;
        this.newMhd = parsedIndex.newMhd;
        this.totalPackedSize = parsedIndex.totalPackedSize;

        try {
            this.volume = this.volumeManager.nextVolume(this, null);
            this.channel = this.volume.getChannel();
    ...
    /** Returns whether this container is one volume of a multi-volume archive. */
    public boolean isMultiVolume() {
        if (this.newMhd != null) {
            return this.newMhd.isMultiVolume();
        }
        for (final BaseBlock block : this.headers) {
            if (block instanceof Rar5MainHeader
                    && ((Rar5MainHeader) block).getRar5Type() == Rar5BlockType.MAIN) {
                return ((Rar5MainHeader) block).isVolume();
            }
        }
        return false;
    }
    ...
    private void addHeader(final BaseBlock header) throws BadRarArchiveException {
        if (this.headers.size() >= this.maxHeaders) {
            throw new BadRarArchiveException(
                    "archive header count exceeds configured limit of " + this.maxHeaders);
        }
